-- Drop old versions of attack_battle function to avoid overload conflicts
-- Keep only the new version with p_result parameter

-- Drop the oldest version (3 parameters)
DROP FUNCTION IF EXISTS public.attack_battle(UUID, INT, TEXT, TEXT);

-- Drop the old version (3 parameters with defaults)
DROP FUNCTION IF EXISTS public.attack_battle(UUID, INT, UUID);

-- The new version with p_result should remain
-- Verify it exists by recreating it (CREATE OR REPLACE is idempotent)
