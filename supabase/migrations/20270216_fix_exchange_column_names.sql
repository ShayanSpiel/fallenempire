-- Fix column name issues in P2P exchange system
-- 1. Fix main_community → main_community_id in order book functions
-- 2. Fix gold_remaining/currency_remaining to use filled_gold_amount calculations

-- =====================================================
-- FIX: GET ORDER BOOK (AGGREGATED) - Correct column names
-- =====================================================
...
BEGIN
  -- Get requesting user's location and community (FIXED: main_community → main_community_id)
  ...
  WHERE o.community_currency_id = p_community_currency_id
    ...
    AND (
      ...
      OR
      -- From allied community (FIXED: main_community → main_community_id)
      EXISTS (
        SELECT 1
        FROM get_active_allies(v_requesting_user_community_id) allies
        WHERE allies.ally_community_id = u.main_community_id
      )
    )
...
-- =====================================================
-- FIX: GET ORDER BOOK (INDIVIDUAL) - Correct column names
-- =====================================================
...
-- =====================================================
-- FIX: CREATE EXCHANGE ORDER - Use filled_gold_amount instead of gold_remaining
-- =====================================================
...
BEGIN
  ...
  BEGIN
    ...
    -- Create exchange order (FIXED: No gold_remaining/currency_remaining columns)
...
-- =====================================================
-- FIX: ACCEPT EXCHANGE ORDER - Use filled_gold_amount instead of gold_remaining
-- =====================================================
...
BEGIN
  ...
  -- Calculate remaining amount (FIXED: use filled_gold_amount instead of gold_remaining)
  ...
  IF p_gold_amount > v_remaining_gold THEN
    RETURN QUERY SELECT NULL::UUID, FALSE, format('Cannot accept more than remaining gold (max: %s)', v_remaining_gold);
    RETURN;
  END IF;
  ...
  -- Update order (FIXED: use filled_gold_amount instead of gold_remaining)
  UPDATE currency_exchange_orders
  SET
    filled_gold_amount = filled_gold_amount + p_gold_amount,
    status = CASE
      WHEN (filled_gold_amount + p_gold_amount) >= gold_amount THEN 'filled'
...
COMMENT ON FUNCTION create_exchange_order IS 'Create a new P2P currency exchange order (FIXED: uses filled_gold_amount)';
COMMENT ON FUNCTION accept_exchange_order IS 'Accept an existing exchange order (FIXED: uses filled_gold_amount)';
