-- Fix job application location validation
-- Previously checked main_community_id instead of actual location (current_hex)
...
BEGIN
  ...
  -- Community members can apply from anywhere; non-members must be currently located in that community's territory
  SELECT EXISTS (
    SELECT 1
  ...
  -- FIXED: Check if user is in community territory using current_hex instead of main_community_id
  SELECT is_user_in_community_territory(p_applicant_id, v_listing.community_id)
