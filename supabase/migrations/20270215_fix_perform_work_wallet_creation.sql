-- Fix perform_work to ensure community_currency_id is always set when creating wallets
-- The issue: v_community_currency_id can be NULL, causing wallet creation to fail
...
BEGIN
  ...
  -- Determine effective community (community that owns the hex)
  ...
  -- CRITICAL FIX: Get community_currency_id from the effective community
  ...
  -- VALIDATION: Ensure we have a valid community_currency_id for wage payments
  ...
  IF v_work_type = 'employee' AND v_wage > 0 THEN
    ...
    -- CRITICAL FIX: Only insert if v_community_currency_id is NOT NULL
    ...
    -- Send tax to community treasury if tax > 0
    IF v_tax_amount > 0 AND v_effective_community_id IS NOT NULL AND v_community_currency_id IS NOT NULL THEN
...
COMMENT ON FUNCTION perform_work IS 'Core production function with work tax deduction. Tax is deducted from wages and sent to community treasury based on work_tax_rate. FIXED: Now properly gets community_currency_id from effective community.';
