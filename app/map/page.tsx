...
export default function MapPage() {
  ...
  const [userCurrentHexName, setUserCurrentHexName] = useState<string | null>(null);
  const [userTicketCount, setUserTicketCount] = useState<number>(0);
  const [userGold, setUserGold] = useState<number>(0);
  const [capitalHexIds, setCapitalHexIds] = useState<string[]>([]);
  ...
  const fetchRegions = useCallback(async () => {
    ...
    (regions ?? []).forEach((row: RawRegionRow) => {
    ...
    if (communityIds.size > 0) {
      const { data: communities } = await supabase
        .from("communities")
        .select("id, name, color, capital_hex_id")
        .in("id", Array.from(communityIds));

      // Map communities by ID for quick lookup
      const communityMap: Record<string, RegionCommunitiesRow | null> = {};
      const capitalSet = new Set<string>();
      (communities ?? []).forEach((c: RegionCommunitiesRow) => {
        communityMap[c.id] = c;
        if (typeof c.capital_hex_id === "string" && c.capital_hex_id.trim()) {
          capitalSet.add(c.capital_hex_id.trim());
        }
      });
      ...
      setCapitalHexIds(Array.from(capitalSet));
    } else {
      setCapitalHexIds([]);
    }
  ...
  const fetchUserLocationAndTickets = useCallback(async (profileId: string) => {
    // Fetch user location
    const { data: locationData } = await supabase.rpc("get_user_location", {
      p_user_id: profileId,
    });
    ...
    const { data: inventory } = await supabase
      .from("user_inventory")
      .select("quantity, resources!inner(key)")
      .eq("user_id", profileId)
      .eq("resources.key", "ticket")
      .maybeSingle();

    if (inventory) {
      setUserTicketCount(Number(inventory.quantity) || 0);
    }

    // Fetch gold from wallets (using PUBLIC user ID)
    ...
    if (wallet) {
      setUserGold(Number(wallet.gold_coins) || 0);
    } else {
      setUserGold(0);
    }
  }, [supabase]);
  ...
  useEffect(() => {
    ...
    async function getUserCommunity() {
      ...
      if (mounted) {
        ...
        // Fetch location and tickets
        void fetchUserLocationAndTickets(profile.id);
      }
  ...
  }, [supabase, router, fetchUserLocationAndTickets]);
  ...
  useEffect(() => {
    ...
    const processPendingRegionUpdates = async () => {
      ...
      if (data && data.length > 0) {
        const typedData = data as RawRegionRow[];
        const ownerIds = Array.from(
          new Set(
            typedData
              .filter((r) => Boolean(r.owner_community_id))
              .map((r) => r.owner_community_id as string),
          ),
        );
        if (ownerIds.length > 0) {
          ...
          typedData.forEach((region) => {
            if (region.owner_community_id) {
              const community = communityMap[region.owner_community_id];
              if (community) {
                region.communities = [community];
              }
            }
          });
          setCapitalHexIds(Array.from(capitalSet));
    ...
    const diplomacyChannel = supabase
      ...
      .on(
        ...
        (payload: any) => {
          if (!mounted) return;
          const record = (payload.new as Partial<{
  ...
  const handleFight = useCallback(async (): Promise<{ battleId?: string }> => {
    ...
    try {
      ...
      if (actionMode === "CLAIM") {
        ...
        const normalizedUserHex = typeof userCurrentHex === "string" ? userCurrentHex.trim() : null;
        const isOnTarget = normalizedUserHex === selectedHex;
        const isOnNeighbor = normalizedUserHex
          ? getHexNeighbors(selectedHex).includes(normalizedUserHex)
          : false;
        if (!isOnTarget && !isOnNeighbor) {
          throw new Error("You must be on this hex or a neighboring hex to attack it.");
        }
        ...
        await Promise.all([fetchRegions(), fetchBattles()]);
        debug(MAP_LOG_MODULE, "Regions and battles refreshed after battle start");
  ...
  const handleTravel = useCallback(
    async (hexId: string) => {
      if (!userId) {
        alert("Please sign in to travel");
        return;
      }

      debug(MAP_LOG_MODULE, "Traveling to hex", { hexId, userId });
      try {
        ...
        if (userId) {
          await fetchUserLocationAndTickets(userId);
        }

        debug(MAP_LOG_MODULE, "Travel successful", result);
      } catch (error) {
        logError("Travel failed:", error);
        throw error;
      }
    },
    [userId, fetchUserLocationAndTickets]
  );

  return (
    <ErrorBoundary section="MapPage" onError={(error) => logError("Map component error:", error)}>
      <div className="fixed inset-0 top-16 z-0 w-screen bg-background overflow-hidden">
        <HexMap
          ...
          drawerUserTicketCount={userTicketCount}
          drawerUserGold={userGold}
          drawerIsFirstClaim={canFirstClaim}
        />
      </div>
    </ErrorBoundary>
  );
