...
interface BattlePassBannerFinalProps {
...
export function BattlePassBannerFinal({ data, onClaimReward }: BattlePassBannerFinalProps) {
  ...
  // Get rewards around current tier (show previous 2, current, and next 7 = ~10 total)
  const startTier = Math.max(1, progress.current_tier - 2);
  const endTier = Math.min(season.total_tiers, progress.current_tier + 7);

  const freeRewards = tiers
    .filter((t) => t.tier_type === "free" && t.tier_number >= startTier && t.tier_number <= endTier)
    .sort((a, b) => a.tier_number - b.tier_number);

  const keeperRewards = tiers
    .filter((t) => t.tier_type === "keeper" && t.tier_number >= startTier && t.tier_number <= endTier)
    .sort((a, b) => a.tier_number - b.tier_number);
        ...
        <div className="border-t border-amber-500/30 bg-gradient-to-b from-amber-500/5 to-background/95">
          <div className="p-4 space-y-3">
                ...
                {freeRewards.map((tier) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className="text-[10px] font-bold mt-0.5 text-foreground">
                        {tier.reward_amount}
                      </span>
                      <div className="absolute -top-2 -left-2 w-5 h-5 rounded-full bg-amber-600 border-2 border-background text-[9px] font-bold text-white flex items-center justify-center shadow-md z-10">
                        {tier.tier_number}
                      </div>
                      {isClaimed && (
                        <div className="absolute inset-0 flex items-center justify-center bg-background/70 rounded-lg">
                          <div className="w-6 h-6 rounded-full bg-green-500 flex items-center justify-center text-white text-xs font-bold shadow-md">
                            ✓
                          </div>
                        </div>
                      )}
            ...
            <div className="space-y-2 opacity-50">
              ...
              <div className="grid grid-cols-5 sm:grid-cols-6 md:grid-cols-8 lg:grid-cols-10 gap-2">
                {keeperRewards.map((tier) => {
                  const icon = getRewardIcon(tier.reward_type, tier.reward_data?.quality_key);

                  return (
                    <div
                      key={`keeper-${tier.tier_number}`}
                      className="relative aspect-square rounded-lg flex flex-col items-center justify-center bg-muted/50 border-2 border-muted-foreground/20 grayscale"
                    >
                      <span className="text-2xl">{icon}</span>
                      <span className="text-[10px] font-bold mt-0.5 text-muted-foreground">
                        {tier.reward_amount}
                      </span>
                      <div className="absolute -top-2 -left-2 w-5 h-5 rounded-full bg-muted border-2 border-background text-[9px] font-bold text-muted-foreground flex items-center justify-center z-10">
                        {tier.tier_number}
                      </div>
                    </div>
                  );
